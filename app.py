...
# --- ১. ডাটাবেস ও সিকিউরিটি (Chat History & Login) ---
conn = sqlite3.connect('skinai_wishy_presentation.db', check_same_thread=False)
c = conn.cursor()
def init_db():
...
# --- ২. এস্থেটিক ডিজাইন ও ব্র্যান্ডিং ---
...
# --- ৪. স্মার্ট মাল্টি-অ্যানসার ও ল্যাঙ্গুয়েজ লজিক ---
def get_intelligent_response(query, res):
    ...
    q = query.lower()
    # স্মার্ট ল্যাঙ্গুয়েজ ডিটেকশন (বাংলা অক্ষর বা বাংলিশ শব্দ)
    is_bangla = any('\u0980' <= char <= '\u09FF' for char in query) or \
                any(word in q for word in ["ki", "keno", "ken", "hoyeche", "korbo", "eta", "osud", "medicine"])
    
    ans = []
    if res == "None": return "দয়া করে আগে একটি ছবি আপলোড করুন।" if is_bangla else "Please upload a photo first."
    ...
    if any(w in q for w in ["keno", "why", "cause", "হলো", "কারণ", "ken"]):
        text = f"{res} মূলত UV রশ্মি বা জীনগত পরিবর্তনের ফলে হয়।" if is_bangla else f"{res} is usually caused by UV radiation or genetic changes."
        ans.append(f"🧬 **{'কারণ' if is_bangla else 'Cause'}:** {text}")
    # ঔষধ চেক
    if any(w in q for w in ["osud", "medicine", "solution", "ঔষধ", "treat", "doctor"]):
        text = "বিশেষজ্ঞ ডাক্তারের পরামর্শ ছাড়া কোনো ঔষধ ব্যবহার করবেন না।" if is_bangla else "Do not use any medication without consulting a dermatologist."
        ans.append(f"⚠️ **{'পরামর্শ' if is_bangla else 'Suggestion'}:** {text}")
    
    return "\n\n---\n\n".join(ans) if ans else (f"আমি রিপোর্টে **{res}** পেয়েছি।" if is_bangla else f"I detected **{res}**.")
...
# --- ৬. সেশন ও সাইডবার (Presentation Mode) ---
    ...
    if not st.session_state.logged_in:
        ...
        with col1:
            if st.button("🔵 Facebook", use_container_width=True):
                st.info("Social Integration Coming Soon!")
        with col2:
            if st.button("🔴 Gmail", use_container_width=True):
                st.info("Direct Google Auth Coming Soon!")
        ...
        with tab2:
            ...
            if st.button("Confirm Sign Up", use_container_width=True):
                ...
                else: st.warning("Enter valid email & 4+ digit password.")
    else:
        st.success(f"Logged in as: {st.session_state.user}")
        if st.button("Logout"): st.session_state.logged_in = False; st.session_state.messages = []; st.rerun()

    st.markdown("---")
    with st.expander("⚙️ Settings"): st.write("v28.0 Production Build")
    with st.expander("❓ Help"): st.write("Upload a clear photo for diagnosis.")
